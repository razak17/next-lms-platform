...
export async function createCourse(data: typeof course.$inferInsert) {
  try {
    const { currentUser } = await getCurrentUser();
    if (!currentUser) return { error: "Unauthorized" };

    // Optional: attach userId if not provided
...
export async function getCourses() {
  try {
    const results = await db.select().from(course);
    return results;
  } catch (error) {
...
export async function getCourseById(courseId: string) {
  try {
    const [result] = await db.select().from(course).where(eq(course.id, courseId));
